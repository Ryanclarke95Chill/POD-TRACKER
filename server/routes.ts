...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/db-status", async (req: Request, res: Response) => {
    try {
      const dbCheck = await pool.query("SELECT COUNT(*) as count FROM consignments");
      const count = parseInt(dbCheck.rows[0].count);
      
      // Also check sync state
      let lastSync = null;
      try {
        const syncState = await pool.query("SELECT last_sync_timestamp FROM axylog_sync_state ORDER BY id DESC LIMIT 1");
        if (syncState.rows.length > 0) {
          lastSync = syncState.rows[0].last_sync_timestamp;
        }
      } catch (e) {
        // Sync state table might not exist
      }
      ...
      return res.json({
        ...
        hasAxylogCreds: !!(process.env.AXYLOG_USERNAME && process.env.AXYLOG_PASSWORD)
      });
    } catch (error: any) {
      return res.json({
        database: "error",
  ...
  // Reset sync state to make live sync worker start fresh
  app.get("/api/reset-sync", async (req: Request, res: Response) => {
    try {
      // Reset sync state to October 6, 2025 to force full resync
      const resetDate = new Date('2025-10-06T00:00:00.000Z');
      
      // First try to update existing sync state
      const updateResult = await pool.query(
        "UPDATE axylog_sync_state SET last_sync_timestamp = $1, updated_at = NOW() RETURNING *",
        [resetDate]
      );
      
      if (updateResult.rows.length === 0) {
        // No existing sync state, create one
        await pool.query(
          "INSERT INTO axylog_sync_state (last_sync_timestamp, is_polling, created_at, updated_at) VALUES ($1, false, NOW(), NOW())",
          [resetDate]
        );
      }
      
      // Clear all existing consignments to start fresh
      await pool.query("DELETE FROM consignments");
      
      return res.json({
        success: true,
        message: "Sync state reset. Live sync worker will automatically start syncing all data within 3 minutes.",
        resetTo: resetDate.toISOString(),
        note: "The dashboard will populate automatically. No manual action needed."
      });
    } catch (error: any) {
      return res.status(500).json({
        success: false,
  ...
  // Force production sync - QUICK VERSION
  app.get("/api/force-sync", async (req: Request, res: Response) => {
    try {
      // Immediate response to prevent timeout
      res.json({
        status: "sync_started",
        message: "Background sync initiated. Check /api/db-status in 30 seconds.",
      ...
      setTimeout(async () => {
        try {
          ...
          if (!username || !password) {
            console.error("No Axylog credentials");
            return;
          }
          ...
          const authResult = await axylogAPI.authenticate();
          if (!authResult) {
            console.error("Axylog authentication failed");
            return;
          }
          ...
          console.log(`Retrieved ${axylogConsignments.length} deliveries`);
          
          if (axylogConsignments.length > 0) {
            // Clear and insert
            await db.delete(consignments);
            
            const consignmentsToInsert = axylogConsignments.map(c => ({
              ...c,
              userId: 1, // Admin user
              syncedAt: new Date()
            }));
            ...
            console.log(`Successfully synced ${axylogConsignments.length} consignments`);
          }
        } catch (error) {
