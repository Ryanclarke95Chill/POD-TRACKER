  ...
  async getConsignmentsWithFilters(filters: {
    ...
    try {
      ...
      deliveries = deliveries.filter((delivery: AxylogDelivery) => {
        ...
        // Check if customer is a Chill depot (exclude depot deliveries)
        const customerName = (delivery.shipToCompanyName || '').toLowerCase();
        const isChillDepotCustomer = customerName.includes('chill') && customerName.includes('depot');
        ...
        const isAutoGeneratedRef = !orderRef || /^\d{4}-\d{5,}-\d+$/.test(orderRef);
