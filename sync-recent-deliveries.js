...
async function syncRecentDeliveries() {
  try {
    console.log('🚀 Syncing Chill Transport deliveries from last 2 days...\n');
    ...
    console.log(`📅 Date range: ${fromDate.split('T')[0]} to ${toDate.split('T')[0]}`);

    // Authenticate with your axylog account
    ...
    console.log(`📦 Found ${deliveries.length} recent deliveries from last 2 days`);

    if (deliveries.length === 0) {
      console.log('ℹ️ No new deliveries in the last 2 days');
      return;
    }
    ...
    try {
      ...
      for (const delivery of deliveries) {
        try {
          ...
          const deliveryDate = new Date(delivery.maxScheduledDeliveryTime || delivery.departureDateTime).toLocaleDateString();
          console.log(`   ✅ ${delivery.year}-${delivery.code}-${delivery.prog} - ${customerName} (${deliveryDate})`);
      ...
      console.log(`\n🎉 Successfully loaded ${inserted} recent consignments from last 2 days!`);
      console.log('📊 Your dashboard now shows only the latest delivery activity.');
